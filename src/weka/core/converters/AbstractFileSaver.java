...
public abstract class AbstractFileSaver extends AbstractSaver implements
		OptionHandler, FileSourcedConverter {
		...
		if (outputString.length() != 0) {
			...
			if (!outputString.endsWith(FILE_EXTENSION)) {
				if (outputString.lastIndexOf('.') != -1)
					outputString = (outputString.substring(0, outputString
							.lastIndexOf('.')))
							+ FILE_EXTENSION;
				else
			...
			try {
				...
				throw new IOException(
						"Cannot create output file. Standard out is used.");
			} finally {
...
*/	}
		...
		if (m_outputFile != null) {
			try {
				if (file.exists()) {
					if (file.delete())
						System.out
								.println("File exists and will be overridden.");
					else
				...
				if (out.lastIndexOf(File.separatorChar) == -1) {
					...
					String outPath = out.substring(0, out
	...
	public void setDirAndPrefix(String relationName, String add) {
		...
		try {
			...
			if (m_prefix.equals(""))
				setFile(new File(m_dir + File.separator + relationName + add
						+ FILE_EXTENSION));
			else
				setFile(new File(m_dir + File.separator + m_prefix + "_"
						+ relationName + add + FILE_EXTENSION));
			setDestination(m_outputFile);
		} catch (Exception ex) {
			System.err
