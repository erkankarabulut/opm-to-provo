 ...
 * @see OptionHandler
 */
public class ModifiedKMeans extends Clusterer implements
		NumberOfClustersRequestable, OptionHandler, WeightedInstancesHandler {
...
*/
	...
	private double distance(Instance first, Instance second) {
		// double[] value1 = new double[first.numAttributes()];
		// double[] value2 = new double[second.numAttributes()];

		// for (int i = 0; i < value1.length; i++) {
		// value1[i] = first.value(i);
		// }
		//
		// for (int i = 0; i < value2.length; i++) {
		// value1[i] = second.value(i);
		// }

		double[] value1 = first.toDoubleArray();
		double[] value2 = second.toDoubleArray();

		double distance = -1;
		try {
			distance = TimeDistance.DFT_distance(value1, value2);
		} catch (Exception e) {
	 ...
	 * 
	 * -N <number of clusters> <br>
	 * Specify the number of clusters to generate. If omitted, EM will use cross
		...
		for (int i = 0; i < m_NumClusters; i++) {
				...
				if (m_ClusterCentroids.attribute(j).isNominal()) {
					temp.append(" "
							+ m_ClusterCentroids.attribute(j).value(
									(int) m_ClusterCentroids.instance(i).value(
											j)));
				} else {
					temp.append(" "
							+ Utils.doubleToString(m_ClusterCentroids.instance(
			...
			for (int j = 0; j < m_ClusterStdDevs.numAttributes(); j++) {
				if (m_ClusterStdDevs.attribute(j).isNumeric()) {
					temp.append(" "
							+ Utils.doubleToString(m_ClusterStdDevs.instance(i)
									.value(j), maxWidth + 5, 4));
				} else {
